...
When inputs are changed in `"view"` mode, it is preferable that the focus (zoom and panning location) does not change. That is where `tmapProxy()` comes into play. It updates the map. Old layer are removed via `tm_remove_layer()` and new layer can be added.
